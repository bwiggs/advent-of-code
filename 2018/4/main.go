...
import (
	...
	"github.com/bwiggs/advent-of-code/2018/4/lib"
...
func main() {
	guards := make(map[string]*SleepSchedule)
	lines := lib.ReadLines("input.txt")
	var currGuardID string
	var sleepStart int
	var sleepyGuard string
	for _, line := range lines {
		s := strings.Split(line, " ")

		min, _ := strconv.Atoi(s[1][3:5])

		switch s[2] {
		case "Guard":
			currGuardID = s[3]
			if _, ok := guards[currGuardID]; !ok {
				guards[currGuardID] = &SleepSchedule{}
			}
		case "falls":
			sleepStart = min
		case "wakes":
			guards[currGuardID].TotalSleepMins += (min - sleepStart)
			for i := sleepStart; i < min; i++ {
				guards[currGuardID].MinuteStats[i]++
			}

			if sg, ok := guards[sleepyGuard]; !ok || guards[currGuardID].TotalSleepMins > sg.TotalSleepMins {
				sleepyGuard = currGuardID
			}
		}
	}

	mostSleepyMinute := 0
	for min, agg := range guards[sleepyGuard].MinuteStats {
		if agg > guards[sleepyGuard].MinuteStats[mostSleepyMinute] {
	...
	for gid, guard := range guards {
		fmt.Printf("%6s %d\n", gid, guard.TotalSleepMins)
	}

	// PART 1

	// Find the guard that has the most minutes asleep. What minute does that guard
	// spend asleep the most? What is the ID of the guard you chose multiplied by
	// the minute you chose? (In the above example, the answer would be 10 * 24 =
	// 240.)

	gid, _ := strconv.Atoi(sleepyGuard[1:])
	fmt.Printf("Part 1: %d\n", gid*mostSleepyMinute)
	...
	for gid := range guards {
		for min, timesAsleep := range guards[gid].MinuteStats {
	...
	gid, _ = strconv.Atoi(guard[1:])
	fmt.Printf("Guard: %s minute: %d Answer: %d\n", guard, minute, gid*minute)

}

// --- Day 4: Repose Record --- You've sneaked into another supply closet - this
// time, it's across from the prototype suit manufacturing lab. You need to
// sneak inside and fix the issues with the suit, but there's a guard stationed
